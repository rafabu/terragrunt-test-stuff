dependency "unit_02" {
  ...
  config_path = format("%s/../../parents/unit_02", replace(get_original_terragrunt_dir(), "\\", "/"))
...
inputs = {
  ...
  unit_03_dependency_unit_02_config_path = format("%s/../../parents/unit_02", replace(get_original_terragrunt_dir(), "\\", "/"))
}